// Task 5 : Solution 1 : Thresholding, any color can be used as a key
...
void Experiment5(int index, int redKey, int greenKey, int blueKey)
{
  ...
  String solution1_extension = "_sol1";
  ...
  for (int i=0; i < imgCf.width * imgCf.height; i++)
  {
    ...
    if(diffRed + diffGreen + diffBlue <= 0.01)
    {
      a = 0.0;
    }else{
      a = 1.0;
  ...
  save(filename + "_exp5_summary" + int(red(colorKey)) + "_" + int(green(colorKey)) + "_" + int(blue(colorKey)) + extension);
}
